<?php
// app/views/consultations/edit.php

// Función old() al inicio
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 pt-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Editar Consulta #<?= $consultation->getIdConsultation() ?></li>
                </ol>
            </nav>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-warning text-dark py-3">
                    <h5 class="mb-0">
                        <i class="bi bi-pencil-square me-2"></i>Editar Consulta Médica
                    </h5>
                </div>
                    ...
                    <form method="POST" action="<?= BASE_URL ?>consultations.php?action=update" autocomplete="off">
                                ...
                                <select name="id_pet" class="form-select" required>
                                    ...
                                    <?php foreach ($pets as $pet): ?>
                                        <option value="<?= $pet['id_pet'] ?>"
                                            <?= (($old['id_pet'] ?? $consultation->getIdPet()) == $pet['id_pet']) ? 'selected' : '' ?>>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.01" name="weight" class="form-control"
                                    value="<?= htmlspecialchars($old['weight'] ?? $consultation->getWeight()) ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Temperatura (°C)</label>
                                <input type="number" step="0.1" name="temperature" class="form-control"
                                    value="<?= htmlspecialchars($old['temperature'] ?? $consultation->getTemperature()) ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Próxima Visita</label>
                                <input type="date" name="next_visit" class="form-control"
                                    value="<?= htmlspecialchars($old['next_visit'] ?? $consultation->getNextVisit()) ?>">
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12">
                                ...
                                <textarea name="diagnosis" class="form-control" rows="3" required><?= htmlspecialchars($old['diagnosis'] ?? $consultation->getDiagnosis()) ?></textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-bold">Tratamiento</label>
                                <textarea name="treatment" class="form-control" rows="3"><?= htmlspecialchars($old['treatment'] ?? $consultation->getTreatment()) ?></textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label">Observaciones</label>
                                <textarea name="observations" class="form-control" rows="2"><?= htmlspecialchars($old['observations'] ?? $consultation->getObservations()) ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 border-top pt-4 mt-4">
                            ...
                            <button type="submit" class="btn btn-warning px-5 shadow-sm">
                                <i class="bi bi-save me-2"></i>Actualizar Consulta
                            </button>
                        </div>
                    </form>
                </div>
